 ...
 *  Entry in vocabulary
 */
private case class VocabWord(     //定义VocabWord -------------样例类
  var word: String,
  var cn: Int,
  var point: Array[Int],  // 这两个是做什么的？
  var code: Array[Int],   // 这两个做什么的？
  var codeLen: Int
)

/**
 * Word2Vec creates vector representation of words in a text corpus.    
 * The algorithm first constructs a vocabulary from the corpus                    1.根据语料库建字典
 * and then learns vector representation of words in the vocabulary.              2.学习字典中词的向量表示
 * The vector representation can be used as features in                           
 * natural language processing and machine learning algorithms.
 *
 * We used skip-gram model in our implementation and hierarchical softmax         1.使用skip-gram算法和层次softmax训练模型
...
class Word2Vec extends Serializable with Logging {
  ...
  private var vectorSize = 100    //这种默认值的写法，如果set里面没用到，这里就是默认值，如果用到了，就改写
  private var learningRate = 0.025
  private var numPartitions = 1
  private var numIterations = 1
  private var seed = Utils.random.nextLong()
  private var minCount = 5
  private var maxSentenceLength = 1000  //句子最大长度，当句子长度大于1000则将句子分块
  ...
  /**
   * Sets vector size (default: 100).输入向量维度，限制为100
   */
  @Since("1.1.0")
  ...
  /**
   * Sets initial learning rate (default: 0.025).学习率默认为0.025
   */
  @Since("1.1.0")
   ...
   * partition越多，精度越差，所以为了尽可能提高精度，应该限制partition的数量
   */
  @Since("1.1.0")
   ...
   * 一个词至少要出现这些次，才放在word2vec中计算，其他的直接过滤掉
   */
  @Since("1.3.0")
  ...
  @transient private var vocab: Array[VocabWord] = null  //不用序列化？？
  @transient private var vocabHash = mutable.HashMap.empty[String, Int]  //

  private def learnVocab[S <: Iterable[String]](dataset: RDD[S]): Unit = { //首先做一个字典
    val words = dataset.flatMap(x => x)  //这一步效果？？x=>x什么意思？？

    vocab = words.map(w => (w, 1))
      .reduceByKey(_ + _)  //做一个wordcount
      .filter(_._2 >= minCount)  //出现次数低于minCount的词过滤掉
      .map(x => VocabWord(
        x._1,
        x._2,
        new Array[Int](MAX_CODE_LENGTH), //这两个是做什么的？？
        new Array[Int](MAX_CODE_LENGTH),
        0))
      .collect()   //把RDD的词典拿到本地来做,去RDD
      .sortWith((a, b) => a.cn > b.cn)  //按照词的数量排序

    vocabSize = vocab.length  //求词典的长度
    require(vocabSize > 0, "The vocabulary size should be > 0. You may need to check " +
      "the setting of minCount, which could be large enough to remove all your words in sentences.")  //判断vocabulary词的数量是否大于0
  ...
  private def createBinaryTree(): Unit = {    //如何建哈夫曼树
   ...
   */
  @Since("1.1.0")
  def fit[S <: Iterable[String]](dataset: RDD[S]): Word2VecModel = {  //模型训练部分
    ...
    val bcVocabHash = sc.broadcast(vocabHash)  //把词典和词序的hash广播出来
    // each partition is a collection of sentences,  -----这个partition是怎么设定的
