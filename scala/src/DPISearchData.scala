...
object DPISearchData {
  ...
  def getDPIFromHDFS = (sc:SparkContext) => {
    ...
    val DPI_raw = sc.textFile("/data/hjpt/edm/fix/itf_fix_dpi/jiangsu/20170329/*/*.gz").coalesce(200).map(line => line.split('|'))
//    val DPI_raw = sc.textFile("")
    val account_url = DPI_raw.filter(line => line.size>27).map(line => (line(0))).distinct.count()

    val account_url_output = account_url.map(line => (line._1 + "\t" + line._2 + '\t' + line._3 + '\t' + line._4 + '\t' + line._5 + '\t' + line._6 + '\t' + line._7) )

    account_url_output.sample(false, 0.001).repartition(1).saveAsTextFile("hdfs:///user/dm/asy/jiangsu_dpi_2017032730days")
